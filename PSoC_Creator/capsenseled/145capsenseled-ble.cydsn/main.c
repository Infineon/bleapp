...
uint8 redLedState = 0; // start it as off
...
void updateLed()
{
    ...
    tempHandle.attrHandle = CYBLE_LEDCAPSENSE_LED_CHAR_HANDLE;
  	tempHandle.value.val = (uint8 *) &redLedState;
        ...
        case CYBLE_EVT_GATTS_WRITE_REQ:
            ...
            if(wrReqParam->handleValPair.attrHandle == CYBLE_LEDCAPSENSE_LED_CHAR_HANDLE)
            {
                ...
                if(CYBLE_GATT_ERR_NONE == CyBle_GattsWriteAttributeValue(&wrReqParam->handleValPair, 0, &cyBle_connHandle, CYBLE_GATT_DB_PEER_INITIATED))
                {
                    ...
                    redLedState = wrReqParam->handleValPair.value.val[0];
                    uart_SpiUartWriteTxData(redLedState);
